...
const CamBlogDetails = () => {
  const { title } = useParams(); // Get blog title from URL (instead of id)
  const location = useLocation(); // Get state data from navigation
  const blog = location.state?.blog; // Retrieve blog data passed via state
