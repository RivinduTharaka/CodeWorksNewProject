...
import { insertData, selectData } from '../../../../services/dataService'; // Import selectData
...
const countries = [
  ...
  { name: "Palestine, State of", code: "PS", dialCode: "+970" },
  ...
  { name: "Sint Maarten (Dutch part)", code: "SX", dialCode: "+1" },
...
// Function to insert registration data into the database (unchanged)
...
function RegisterEvent() {
  ...
  const [searchTerm, setSearchTerm] = useState(''); // State for filtering country codes
  const [remainingSeats, setRemainingSeats] = useState(null); // State for remaining seats
  const [loadingSeats, setLoadingSeats] = useState(true); // State for loading seats
  const [errorSeats, setErrorSeats] = useState(null); // State for error in fetching seats

  // Fetch remaining seats from the database
  useEffect(() => {
    const fetchRemainingSeats = async () => {
      ...
      try {
        const response = await selectData('events', { id: parseInt(id), is_active: true });
        if (response.data && response.data.length > 0) {
          const eventData = response.data[0];
          setRemainingSeats(eventData.remaining_seats.toString()); // Convert to string to match existing logic
        } else {
          setErrorSeats('Event not found in the database.');
        }
      } catch (error) {
        console.error('Error fetching remaining seats:', error);
        setErrorSeats('Failed to fetch remaining seats. Please try again.');
      } finally {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'countryCode') {
      ...
      setSearchTerm(''); // Reset search term when a selection is made
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    // Use the dynamically fetched remainingSeats instead of event.remaining_seats
    if (remainingSeats === '0' || remainingSeats === 'Unlimited') {
      if (remainingSeats !== 'Unlimited') {
        await Swal.fire({
          icon: 'error',
                ...
                <Box sx={{ mt: 2 }}>
                  ...
                  <Typography variant="subtitle1" sx={{ color: theme.palette.text.secondary, mb: 1 }}>
                    ...
                    {loadingSeats ? (
                      ...
                      <span style={{ color: 'red' }}>{errorSeats}</span>
                    ) : (
                      remainingSeats
                    )}
                  </Typography>
