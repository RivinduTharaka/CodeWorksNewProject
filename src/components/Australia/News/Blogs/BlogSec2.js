...
const BlogSec2 = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const blogsCountryResponse = await selectData('blog_countries', { is_active: true, country_id: 2 });
      console.log('Fetched blogs:', blogsCountryResponse.data || []);      // Fetch blogs directly from the blogs table
      const blogsResponse = await selectData('blogs', { is_active: true , id: blogsCountryResponse.data.blog_id });
